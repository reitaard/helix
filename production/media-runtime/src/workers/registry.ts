...
export class WorkerRegistry {
  ...
  private readonly adapters = new Map<string, ComfyAdapter>();

  constructor(workers: WorkerDefinition[]) {
    for (const worker of workers) {
      this.definitions.set(worker.id, worker);
      if (worker.adapter === "comfy") {
        this.adapters.set(worker.id, new ComfyAdapter(worker.endpoint));
      }
    }
  }
  ...
  private summary(worker: WorkerDefinition) {
    return {
      ...
      modelFamilies: Object.fromEntries(worker.productionProfiles.map(profile => [profile.id, profile.modelFamilies])),
      maxConcurrentGpuJobs: worker.maxConcurrentGpuJobs
    };
  }

  list() { return [...this.definitions.values()].map(worker => this.summary(worker)); }
  get(id: string) { const worker = this.definitions.get(id); return worker ? this.summary(worker) : null; }
  ...
  resolveProfile(workerId: string, tool: string, profileId?: string): ProfileResolution {
    ...
    if (matches.length === 0) return { kind: "profile_ambiguous", tool, profileIds: [] };
    return { kind: "profile_ambiguous", tool, profileIds: matches.map(profile => profile.id) };
  }
  ...
  async liveness(id: string) {
    ...
    return { workerId: worker.id, name: worker.name, revision: worker.revision, ...(await adapter.liveness()) };
  }
  ...
  async queue(id: string) { return this.adapters.get(id)?.queueSummary() ?? null; }
  async submit(id: string, workflow: Record<string, unknown>) { return this.adapters.get(id)?.submit(workflow) ?? null; }
  async cancel(id: string, backendJobId: string) { return this.adapters.get(id)?.cancel(backendJobId) ?? null; }
  async downloadArtifact(id: string, artifact: { filename: string; subfolder: string; type: string }, destinationPath: string) { const adapter = this.adapters.get(id); if (!adapter) return false; await adapter.downloadArtifact(artifact, destinationPath); return true; }
