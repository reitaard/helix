...
export class T2IPendingRepository {
  ...
  async beginPrompt(chatId: string, expiresAt: Date) {
    await this.db.query(
      `
  ...
  async incrementInvalid(chatId: string): Promise<PendingT2I | null> {
    const result = await this.db.query<PendingRow>(
      `
      UPDATE operator_pending_t2i
      SET
  ...
  async remove(chatId: string) {
    await this.db.query(
      `DELETE FROM operator_pending_t2i WHERE chat_id = $1`,
      [chatId]
    );
  }
  ...
  async expireDue(chatId: string) {
    const result = await this.db.query(
      `DELETE FROM operator_pending_t2i WHERE chat_id = $1 AND expires_at <= NOW()`,
      [chatId]
    );
